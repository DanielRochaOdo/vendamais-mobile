...
const getPlanoValorByRegra = (plano: any, regraValor?: string) => {
  if (!plano) return 0;
...
export function InclusaoDependenteModal({ onClose, onSuccess }: InclusaoDependenteModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-2 sm:p-4 z-50 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-xl max-w-5xl w-full my-2 sm:my-8 max-h-[95vh] sm:max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-slate-200 px-4 sm:px-6 py-3 sm:py-4 flex items-start sm:items-center justify-between gap-3 z-10">
          <div>
            <h2 className="text-lg sm:text-xl font-bold text-slate-800">Inclusão de Dependente</h2>
            ...
            <>
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
                  ...
                  <Button onClick={handleAdicionarDependente} variant="secondary" className="w-full sm:w-auto">
                      ...
                      <div key={index} className={`bg-white border rounded-lg p-4 ${dep.saved ? 'border-green-500' : 'border-emerald-200'}`}>
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
                          ...
                          <button
                            ...
                            className="p-1 text-red-600 hover:bg-red-50 rounded transition-colors self-end sm:self-auto"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          ...
                          <Select
                            ...
                            onChange={(e) => {
                              ...
                              const planoEmpresaSelecionado = planosEmpresa.find(
                                (p: any) => getPlanoId(p) === planoSelecionado
                              );

                              const planoMap = planos.find((pm) => Number(pm.plano_id) === planoSelecionado);

                              const valorPlano = getPlanoValorByRegra(planoEmpresaSelecionado, planoMap?.regra_valor);
                          ...
                          >
                            ...
                            {planosEmpresa
                              .filter((p: any) => !config?.planos_ocultos?.includes(getPlanoId(p).toString()))
                              .map((planoEmpresa: any, idx) => {
                                const planoId = getPlanoId(planoEmpresa);
                                const planoMap = planos.find((pm) => Number(pm.plano_id) === planoId);
                                const nomePlano =
