...
export function DependentesSection({
  ...
  const parsePlanoValor = (plano: any, tipo: 'titular' | 'dependente') => {
    const valorBruto =
      tipo === 'titular'
        ? plano?.ValorTitular ?? plano?.valorTitular ?? plano?.valor_titular
        : plano?.ValorDependente ?? plano?.valorDependente ?? plano?.valor_dependente;
  ...
  const getPlanoId = (plano: any) => {
    ...
    const planoId = Number(planoRaw);
    return Number.isFinite(planoId) ? planoId : 0;
  };

  const getPlanoNome = (plano: any, planoId: number) =>
    plano?.nomeExibicao ||
    plano?.nome_exibicao ||
    plano?.NomeANS ||
    plano?.nomeANS ||
  ...
  const planosNormalizados: NormalizedPlano[] = planos
    .map((plano, index) => {
      const planoId = getPlanoId(plano);
      if (!planoId) return null;

      return {
        key: `${planoId}-${index}`,
        id: planoId,
        nome: getPlanoNome(plano, planoId),
        valorTitular: parsePlanoValor(plano, 'titular'),
        valorDependente: parsePlanoValor(plano, 'dependente'),
      };
    })
    .filter((plano): plano is NormalizedPlano => Boolean(plano));

  const planosFiltrados = planosNormalizados.filter((plano) => {
    const planoId = plano.id.toString();
  ...
  const handleAdd = () => {
    ...
    const planoId = parseInt(formData.plano, 10);
    const planoSelecionado = planosNormalizados.find((p) => p.id === planoId);
    if (!planoSelecionado) {
    ...
    const valorPlanoSelecionado = planoSelecionado.valorDependente;
    ...
    <div className="border-t border-slate-200 pt-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        ...
        {!isAdding && (
          <button
            ...
            className="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white rounded-lg transition-colors w-full sm:w-auto"
            style={{
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
              ...
              {planosFiltrados.map((plano) => {
                const valorTitular = formatarValorPlano(plano.valorTitular);
                const valorDependente = formatarValorPlano(plano.valorDependente);

                return (
                  <option key={plano.key} value={plano.id}>
                    {plano.nome} - Titular: R$ {valorTitular} / Dep: R$ {valorDependente}
                  </option>
                );
              })}
            </Select>
      ...
      <div className="space-y-3">
        {dependentes.map((dep, index) => {
          ...
          const plano = planosNormalizados.find((p) => p.id === planoNumero);
          ...
          return (
            <div
              key={index}
              className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between p-4 bg-slate-50 rounded-lg border border-slate-200"
            >
                ...
                <div className="text-sm text-slate-600 space-y-0.5">
                  ...
                  <p>
                    ...
                    {temPlanoValido && (
                      <>
                        {' • Plano: '}
                        {plano?.nome || `Plano ${planoNumero}`} (R$ {dep.planoValor})
                      </>
                    )}
                    {!temPlanoValido && (
              ...
              <div className="flex w-full sm:w-auto flex-col sm:flex-row gap-2">
                <button
                  ...
                  className={`inline-flex items-center justify-center gap-2 rounded-lg px-3 py-2 text-xs font-medium transition-colors w-full sm:w-auto ${
                    temPlanoValido
                ...
                <button
                  ...
                  className="p-2 hover:bg-red-100 rounded transition-colors self-end sm:self-auto"
                  title="Excluir"
                >
