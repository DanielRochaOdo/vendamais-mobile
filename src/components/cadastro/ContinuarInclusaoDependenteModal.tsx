...
type DraftContinuarInclusao = {
  ...
  empresaNome?: string;
  empresaObservacao?: string;
  planosEmpresa?: Array<Record<string, unknown>>;
};

type EmpresaRawData = Record<string, unknown> & {
  id?: unknown;
  razaoSocial?: unknown;
  nomeFantasia?: unknown;
  cnpj?: unknown;
  enderecoEmpresa?: unknown;
  precoPlano?: unknown;
  exigeMatricula?: unknown;
  observacao?: unknown;
  observacoes?: unknown;
};
...
const toEmpresaRaw = (value: unknown): EmpresaRawData | null => (
  typeof value === 'object' && value !== null ? (value as EmpresaRawData) : null
);
...
const toPlanoNumber = (value: unknown): number => {
...
const getPlanoId = (plano: Record<string, unknown> | null | undefined) =>
  toPlanoNumber(
    plano?.Plano ??
    plano?.plano ??
    plano?.plano_id ??
...
const getPlanoValorByRegra = (plano: Record<string, unknown> | null | undefined, regraValor?: string) => {
  if (!plano) return 0;

  if (regraValor === 'titular') {
    return toPlanoNumber(plano?.ValorTitular ?? plano?.valorTitular ?? plano?.valor_titular);
  }

  if (regraValor === 'agregado') {
    return toPlanoNumber(plano?.ValorAgregado ?? plano?.valorAgregado ?? plano?.valor_agregado);
  }

  return toPlanoNumber(plano?.ValorDependente ?? plano?.valorDependente ?? plano?.valor_dependente);
};
...
export function ContinuarInclusaoDependenteModal({ cadastro, onClose, onSuccess }: ContinuarInclusaoDependenteModalProps) {
  ...
  const [dependentes, setDependentes] = useState<DependenteForm[]>(() => {
    const dependentesExistentes = toRecordArray(cadastro.dependentes);

    if (dependentesExistentes.length > 0) {
      return dependentesExistentes.map((dep) => ({
        id: crypto.randomUUID(),
        nome: toStringValue(dep.nome),
        cpf: toStringValue(dep.cpf),
        dataNascimento: toStringValue(dep.data_nascimento),
        sexo: dep.sexo === 'Masculino' ? 1 : dep.sexo === 'Feminino' ? 0 : -1,
        parentesco: toPlanoNumber(dep.parentesco),
        plano: toPlanoNumber(dep.plano_codigo),
        planoValor: '0.00',
        nomeMae: toStringValue(dep.nome_mae),
        arquivo: toStringValue(dep.arquivo_path) ? {
          nome: toStringValue(dep.arquivo_path).split('/').pop() || 'Arquivo existente',
          path: toStringValue(dep.arquivo_path),
  ...
  useEffect(() => {
    ...
    const draft = loadDraft('continuar-inclusao-dependente-modal', profile.id, cadastro.id) as DraftContinuarInclusao | null;
    if (draft) {
      ...
      setPlanosEmpresa(toRecordArray(draft.planosEmpresa));
    }
  ...
  useEffect(() => {
    ...
    const empresaRaw = toEmpresaRaw(cadastro.empresa_raw);
    const precoPlano = toRecordArray(empresaRaw?.precoPlano);

    if (empresaRaw && precoPlano.length > 0) {
      setPlanosEmpresa(precoPlano);
      setEmpresaObservacao(toStringValue(empresaRaw.observacao ?? empresaRaw.observacoes));

      setSelectedEmpresa({
        id: cadastro.empresa_codigo || toPlanoNumber(empresaRaw.id),
        razaoSocial: toStringValue(empresaRaw.razaoSocial) || cadastro.empresa_nome || '',
        nomeFantasia: toStringValue(empresaRaw.nomeFantasia) || cadastro.empresa_nome || '',
        cnpj: toStringValue(empresaRaw.cnpj),
        enderecoEmpresa: toRecord(empresaRaw.enderecoEmpresa),
        precoPlano,
        exigeMatricula: toPlanoNumber(empresaRaw.exigeMatricula),
        observacoes: toStringValue(empresaRaw.observacao ?? empresaRaw.observacoes),
        raw: empresaRaw,
      });

      if (cadastro.plano_codigo && precoPlano.length > 0) {
        const planoEncontrado = precoPlano.find((p) => getPlanoId(p) === cadastro.plano_codigo);
        if (planoEncontrado) {
          const planoMap = planos.find((pm) => pm.plano_id === cadastro.plano_codigo);
          const valor = getPlanoValorByRegra(planoEncontrado, planoMap?.regra_valor);
    ...
    if (planosEmpresa.length > 0) {
      setDependentes(prev => prev.map(dep => {
        if (dep.plano && dep.plano !== 0) {
          const planoEncontrado = planosEmpresa.find((p) => getPlanoId(p) === dep.plano);
          if (planoEncontrado) {
    ...
    try {
      ...
      if (result.ok && result.empresas && result.empresas.length > 0) {
        ...
        if (cadastro.plano_codigo && empresa.precoPlano) {
          const planoEncontrado = empresa.precoPlano.find((p: Record<string, unknown>) => getPlanoId(p) === cadastro.plano_codigo);
          if (planoEncontrado) {
            const planoMap = planos.find((pm) => pm.plano_id === cadastro.plano_codigo);
            const valor = getPlanoValorByRegra(planoEncontrado, planoMap?.regra_valor);
  ...
  const handlePlanoChange = (index: number, planoId: number) => {
    const planoEncontrado = planosEmpresa.find((p) => getPlanoId(p) === planoId);
  ...
  const updateDependente = (index: number, field: keyof DependenteForm, value: DependenteForm[keyof DependenteForm]) => {
    setDependentes(prev => prev.map((dep, idx) =>
  ...
  const handleSalvarRascunho = async () => {
    ...
    try {
      ...
      setError(toErrorMessage(err, 'Erro ao salvar rascunho'));
    } finally {
      setSalvando(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    for (let i = 0; i < dependentes.length; i++) {
      ...
      if (config?.exigir_arquivo && !dep.arquivo && !(i === 0 && cadastro.arquivo_path)) {
  ...
  const handleEmpresaModalSelected = async (codigo: number, nome: string, planos: Array<Record<string, unknown>>) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-emerald-600 text-white px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between rounded-t-xl z-10">
          <h2 className="text-lg sm:text-xl font-bold">Continuar Inclusão de Dependentes</h2>
          <button
              ...
              <div key={dep.id} className="mb-6 p-4 bg-slate-50 rounded-lg border border-slate-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    {planosEmpresa
                      .filter((p) => !config?.planos_ocultos?.includes(getPlanoId(p).toString()))
                      .map((p, idx: number) => {
                        const planoId = getPlanoId(p);
                        const planoMap = planos.find((pm) => pm.plano_id === planoId);
                        const valor = getPlanoValorByRegra(p, planoMap?.regra_valor);
                        const nomePlano =
                          toStringValue(
                        ...
                        return (
                          <option key={`${planoId}-${idx}`} value={planoId}>
                            {nomePlano} - R$ {valor.toFixed(2)}
                          </option>
                        );
                      })}
                  </Select>
        ...
        <div className="sticky bottom-0 bg-slate-50 px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-t border-slate-200 rounded-b-xl">
          <Button
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            <Button onClick={handleCloseModal} variant="secondary" disabled={loading || salvando} className="w-full sm:w-auto">
              Cancelar
            </Button>
            <Button
